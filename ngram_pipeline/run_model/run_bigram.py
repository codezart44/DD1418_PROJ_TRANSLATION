...
class BigramModel:
    def __init__(self, bigram_file: str, monogram_file: str, lexicon: dict) -> None:
        self.lexicon = lexicon

        self.monogram_probs = dict()
        self.bigram_probs = defaultdict(dict)
        self.__init_model(bigram_file=bigram_file, monogram_file=monogram_file)

    
    def __init_model(self, bigram_file, monogram_file):
        ...
        # print(self.bigram_probs["frequency"]["of"])
        # quit()
    ...
    def use_bigram(self, prev_trn: str, swe_word: str) -> str:
        ...
        trn_probs = [int(self.bigram_probs.get(prev_trn, {}).get(trn.lower(), 0)) for trn in translations]  # NOTE actually counts atm
    ...
    def translate(self, text: str) -> str:
        ...
        input_words = ["<SOS>"] + text.lower().strip().split(" ")
        output_words = ["<SOS>"]

        for i in range(1, len(input_words)):
            prev_trn = output_words[i-1]
            swe_word = input_words[i]
            trn = self.use_bigram(prev_trn=prev_trn, swe_word=swe_word)
...
def main():
    ...
    for i, sentence in enumerate(swe_sentences2):
        lexical_translation = lexical_translate(lexicon=sv_en_lexicon, sentence=sentence)
        bigram_translation = bigram_model.translate(text=sentence)

        printf_translation(src_sentence=sentence, trn_sentence=eng_sentences2[i], tgt_sentence=lexical_translation)
        printf_translation(src_sentence=sentence, trn_sentence=eng_sentences2[i], tgt_sentence=bigram_translation)
